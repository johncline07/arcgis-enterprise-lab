...
- **the greater Terraform realization:** — import did less magic than I initially expected. At the time of using `import` I thought that if I had existing infrastructure in Azure, then importing it would magically write the HCL that represents the resources. How woefully and tragically naive I was. I didn't have a perfect "aha" moment, it took a few hours of researching `import` and thinking relatively deeply **(congrats, to those of you who it was made clear immediately)** about it to sort of start getting it. More reps writing HCl, it came to me that `import` tells Terraform that resources exist, but you still have to **describe it in HCL**. 
